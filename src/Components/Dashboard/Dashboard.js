...
class Dashboard extends Component {
    ...
    getPosts(id) {
        if (this.state.userposts === true) {
            ...
            axios.get(`/api/posts2/${this.props.user.id}`).then((res) => {
                this.setState({
                    posts: res.data
                })
            })
    ...
    render() {
        // let url=`/post/${e.id}`
    const newArray = this.state.posts.map((e, index) => ([<Link key={index} to={`/post/${e.id}`} > <div className="postsdiv" ><div className="picdiv"><h2>{e.title}</h2></div><div className="authordiv"><h6>by {e.username}</h6><img className="postspic" src={e.profile_pic} alt="profile" /></div></div></Link >]))
        return (
